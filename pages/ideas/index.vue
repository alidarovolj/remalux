...
import IdeaShortCard from "~/components/cards/ideaShortCard.vue";
    ...
    <div class="container mx-auto px-4 lg:px-0">
      <main class="mx-auto">
        <div class="pb-24">
          ...
          >
            <div
              class="flex gap-5 flex-col md:flex-row justify-between items-center mb-9"
            >
              <p class="text-lg md:text-2xl font-montserrat">
                {{ $t("ideas.direction") }}
              </p>
              <select
                v-if="ideaRoomsList"
                @change="updateRoomFilter"
                class="w-full md:w-1/3 p-2 md:p-4 text-sm md:text-base rounded-lg border"
              >
                <option :value="null">
                  {{ $t("ideas.select_room") }}
                </option>
            ...
            <div v-if="ideaColorsList" class="mb-10">
              <div
                v-if="ideaColorsList.data.length > 0"
                class="grid md:flex grid-cols-5 gap-y-3"
              >
                <div
                  ...
                  class="color-block w-full h-[70px] md:h-auto gap-y-2"
                  :class="{
                    selected:
                      parseInt(route.query['filters[ideaColor.id]']) ===
                      item.id,
                  }"
                >
                  <CheckCircleIcon
                    v-if="
                      parseInt(route.query['filters[ideaColor.id]']) === item.id
                    "
            ...
            <div v-if="ideasList">
              <div
                ...
                class="grid grid-cols-2 gap-y-5 sm:grid-cols-3 sm:gap-x-5 sm:gap-y-10 xl:grid-cols-3 gap-x-3"
              >
                <IdeaShortCard
