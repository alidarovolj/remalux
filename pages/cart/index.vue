...
const checkedCartCookie = computed(
  () =>
    ...
    cartList.value.data.length === cartCookie.value.length
);
    ...
    <div class="container mx-auto px-4 md:px-0">
      <div class="mt-12 mb-32 pb-10 rounded-2xl shadow-lg">
        <div class="flex justify-between px-5 md:px-10">
          <h1 class="text-lg md:text-xl font-medium font-montserrat">
            {{ t("cart.title") }}:
            <span v-if="cartCookie">{{ cartCookie.length }}</span
            >)
          </h1>
          <div v-if="cartList">
            <div
            ...
            >
              <p class="text-sm md:text-base font-medium text-red-500">
                {{ t("cart.clear") }}
              </p>
            </div>
          </div>
        ...
        <div class="mt-8 flow-root">
          ...
          >
            <div
              v-if="cartList.data.length > 0"
              class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8 mb-8 text-xs md:text-base"
            >
                    ...
                    <td class="whitespace-nowrap px-3 py-5">
                      <div v-if="item.product_variant.product.is_colorable">
                        ...
                        >
                          ...
                          ></div>
                          <p
                            class="opacity-0 transition-all absolute left-0 text-center top-[105%]"
                          >
                            {{ item.color_id.title[cur_lang] }}
                          </p>
                        </div>
                        <div
                          v-else
        ...
        <div class="flex justify-end pr-10">
          <NuxtLink
            ...
            class="block w-max px-6 py-2 border hover:bg-mainColor hover:text-white transition-all text-center border-mainColor text-mainColor rounded-xl text-sm md:text-base font-medium"
          >
            {{ $t("cart.checkout.checkout_button") }}
          </NuxtLink>
          <p
            v-else
            class="block w-max px-6 py-2 border bg-gray-200 text-center border-gray-200 cursor-not-allowed rounded-xl text-sm md:text-base font-medium"
            @click="
              notifications.showNotification(
                'error',
                'Ошибка',
