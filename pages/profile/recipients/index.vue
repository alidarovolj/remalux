      ...
      <div class="bg-white p-4 md:p-6 rounded-2xl set_shadow">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-lg md:text-xl font-medium">
              {{ $t("profile.recipients") }}
            </h1>
          </div>
        </div>
        <div v-if="recipientList">
          <div v-if="recipientList.data.length > 0" class="flex flex-col gap-5">
            <div
        ...
        >
          <PlusIcon class="w-4 md:w-7 h-4 md:h-7" />
          <p class="text-sm md:text-base">{{ $t("profile.add_recipient") }}</p>
        </div>
      </div>
    </NuxtLayout>
  </div>
