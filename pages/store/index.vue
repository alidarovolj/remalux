...
const chosenCategory = computed(() => {
  if (route.query["filters[product.category_id]"]) {
    return categories.categoriesList?.data?.find(
      (category) =>
        category.id === parseInt(route.query["filters[product.category_id]"])
    );
  }
  return null;
});
...
const updateFilter = async (sectionId, optionId) => {
  ...
  if (newQuery[filterKey].includes(optionId.toString())) {
    newQuery[filterKey] = newQuery[filterKey].filter(
...
const filterElements = computed(() => {
  ...
  if (
    ...
    Array.isArray(filtersStore.filtersList.data)
  ) {
...
onMounted(async () => {
  ...
  if (colorCookie.value) {
    await router.push({
      query: { ...route.query, color_id: colorCookie.value.id },
    });
  }
  await nextTick();
...
<template>
  <client-only>
    <div>
      ...
      <div v-else>
        <div class="container mx-auto px-4 md:px-0">
          <div class="my-0 md:my-8">
            ...
            >
              <div
                class="flex flex-col md:flex-row items-center gap-3 md:gap-6 mb-4"
              >
                <div class="flex gap-2 items-center">
                  <p class="text-sm md:text-2xl font-medium whitespace-nowrap">
                    {{ colorCookie.title[cur_lang] }}
                  </p>
                  <ChevronRightIcon class="w-3 md:w-6 h-3 md:h-6" />
                </div>
                ...
                >
                  <p class="whitespace-nowrap font-medium text-sm md:text-base">
              ...
              >
                ...
                >
                  <svg
                    :class="{
                      'text-mainColor': favouriteColorIds?.includes(
                        colorCookie.id
                      ),
                    }"
      ...
      <div
        class="container mx-auto px-4 lg:px-0 pb-16 pt-10 md:pt-0 hidden md:block"
      >
        <div
          v-if="categories.categoriesList"
          class="md:grid md:grid-cols-4 lg:gap-x-5 lg:gap-y-5 flex-col gap-2 font-manrope"
        >
          <div
            v-for="(category, index) in categories.categoriesList.data"
            :key="index"
            :class="{
              'for_gradient text-mainColor':
                parseInt(route.query['filters[product.category_id]']) ===
                category.id,
            }"
      ...
      <div class="container mx-auto px-4 lg:px-0">
        <main class="mx-auto">
          <div
            class="pb-24 lg:grid lg:grid-cols-3 lg:gap-x-8 xl:grid-cols-4 font-manrope"
          >
            <aside>
              <form
                @submit.prevent="searchByText"
                class="mb-5 relative hidden md:block"
              >
                <MagnifyingGlassIcon
                  class="w-6 h-6 text-mainColor absolute left-4 top-1/2 -translate-y-1/2"
                />
                <input
              ...
              <div
                class="hidden md:flex items-center justify-between lg:hidden"
              >
                <button
              ...
              <div class="hidden lg:block border border-[#F0DFDF] rounded-md">
                <div class="flex">
                  <div
                    class="relative ml-auto flex h-full w-full flex-col overflow-y-auto rounded-md pt-4"
                  >
                    <div class="flex items-center justify-between px-4">
                      <h2
                        class="text-2xl font-semibold text-gray-900 font-montserrat"
                      >
                        {{ $t("products.filters") }}
                      </h2>
                      <p
                        ...
                        >
                          <fieldset>
                            <legend class="w-full">
                              ...
                              >
                                <span
                                  class="text-xl text-start font-medium text-gray-900 font-montserrat"
                                >
                                  {{ $t("sorting.colorable") }}
                                </span>
                                <span
                                  class="ml-6 flex h-7 items-center text-black"
                                >
                                  <ChevronDownIcon
                                    :class="[
                                      open ? '-rotate-180' : 'rotate-0',
                                ...
                                <div class="space-y-4 bg-[#F9F9F9] px-4 py-4">
                                  <div class="flex gap-3 items-center">
                                    <input
                                      id="colorable"
                                      :checked="
                                        route.query.is_colorable?.includes(1)
                                      "
                                      name="colorable"
                                      value="0"
                                  ...
                                  <div class="flex gap-3 items-center">
                                    <input
                                      id="non-colorable"
                                      :checked="
                                        route.query.is_colorable?.includes(0)
                                      "
                                      name="non-colorable"
                                      value="0"
                                      class="h-4 w-4 min-w-4 min-h-4 rounded border-gray-300 text-mainColor focus:ring-indigo-500"
                                ...
                                <div class="space-y-4 bg-[#F9F9F9] px-4 py-4">
                                  ...
                                  >
                                    <input
                                      ...
                                      type="checkbox"
                                      @change="
                                        updateFilter(section.id, option.id)
                                      "
                                    />
                                    <label
                                      :for="`${section.id}-${optionIdx}-mobile`"
                        ...
                        >
                          <fieldset>
                            <legend class="w-full">
                              ...
                              >
                                <span
                                  class="text-xl font-medium text-gray-900 font-montserrat"
                                >
                                  {{ $t("cart.table.price") }}
                                </span>
                                <span
            ...
            >
              <div
                class="flex justify-between items-center mb-5 pb-3 border-b border-[#F0DFDF]"
              >
                <p
                  class="text-xl md:text-3xl font-medium text-mainColor font-montserrat"
                >
                  <span v-if="!chosenCategory">{{
              ...
              >
                <div
                  ...
                  :key="index"
                  :class="{
                    'new_grad relative text-mainColor border-mainColor':
                      parseInt(route.query['filters[product.category_id]']) ===
                      category.id,
                  }"
                  class="relative w-full set_shadow rounded flex items-center transition-all cursor-pointer text-[#7B7B7B] hover:shadow-hovShadow border border-[#7B7B7B]"
                  @click="updateCategoryFilter(category.id)"
                >
                  <div class="flex flex-col gap-5 px-2 py-1">
                    <p class="text-sm font-montserrat whitespace-nowrap">
                      {{ category.title[cur_lang] }}
                    </p>
                  </div>
                </div>
              ...
              <form
                @submit.prevent="searchByText"
                class="mb-5 relative block md:hidden mt-3"
              >
                <MagnifyingGlassIcon
                  class="w-5 h-5 text-mainColor absolute left-4 top-1/2 -translate-y-1/2"
                />
                <input
                  v-model="searchForm.searchKeyword"
                  :placeholder="$t('search.placeholder')"
                  class="w-full py-2 pl-12 border-b border-[#F0DFDF] text-sm"
                  type="text"
                />
              </form>
              ...
              <div
                @click="mobileFiltersOpen = true"
                class="flex md:hidden items-center lg:hidden mb-5 bg-mainColor text-center justify-center text-white py-2 rounded"
              >
                <button
              ...
              >
                <div
                  class="w-full text-[#7B7B7B] flex items-center gap-3 mb-5 md:mb-0"
                >
                  ...
                  <div class="flex gap-3 flex-wrap">
                    ...
                    >
                      ...
                      <XMarkIcon
                        ...
                        @click="updateFilter(null, filter.id)"
                      />
                    </div>
                  </div>
                </div>
                <Menu as="div" class="relative inline-block text-left">
                  ...
                  >
                    <MenuItems
                      class="absolute left-0 md:right-0 md:-left-1/2 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                    >
                      <div class="py-1">
