...
const setHelpfulReview = async (form, comment) => {
  if (token.value) {
    ...
    notifications.showNotification(
      "error",
      "Ошибка",
      "Для оценки отзыва необходимо авторизоваться."
    );
  }
};
...
const links = computed(() => [
  ...
  {
    title: detailProduct.value?.title[cur_lang.value],
    link: localePath(`/store/${route.params.id}`),
  },
]);
...
const productReviewsRating = computed(
  () => products.productReviewsRating || {}
);
...
const paintNeeded = computed(() => {
  return (
    (totalArea.value * form.value.layers * detailProduct.value.expense) / 1000
  );
});
...
const dynamicLink = computed(() => {
  return [
    {
      rel: "canonical",
      href: `${t("headers.store.canonical")}/${route.params.id}`,
    },
  ];
});
  ...
  <div class="pb-10 md:pb-32">
        ...
        <div v-if="detailProduct && productsList">
          <div class="flex flex-col md:flex-row items-start gap-7">
            <div
              :class="{
                'border-none':
                  colorCookie || !products.detailProduct.is_colorable,
              }"
            ...
            >
                ...
                <client-only>
                  ...
                  >
                    ...
                    >
                      <div
                        :class="[
                          { 'bg-white': !products.detailProduct.is_colorable },
                        ]"
                    ...
                    <template #addons="{ currentSlide, slidesCount }">
                      ...
                      >
                        <div
                          class="hidden md:block object-contain h-max z-20 bg-white rounded-tr-lg w-[100px] min-w-[100px]"
                        >
                          <img
                        ...
                        <div
                          ...
                          :style="`background: ${colorCookie.hex}`"
                          :class="{
                            'border-mainColor': currentSlide === index,
                          }"
              ...
              <div v-else class="w-full h-full product-carousel">
                <client-only>
                  <my-carousel-carousel
                    ref="imagesCarousel"
                    :autoplay="4000"
            ...
            >
              <div
                class="absolute right-8 top-8 w-8 h-8 rounded-full bg-[#F0DFDF] flex items-center justify-center"
              >
                <svg
              ...
              <div class="flex items-center gap-2 mb-4">
                ...
                <h1
                  class="text-lg md:text-3xl font-medium font-montserrat w-[90%]"
                >
                  {{ detailProduct.title[cur_lang] }}
                </h1>
              </div>
              <p class="text-[10px] md:text-sm text-[#7B7B7B] mb-8">
                {{ $t("products.details.article") }}:
              ...
              <div class="flex justify-between items-center gap-1 mb-8">
                ...
                >
                  ...
                  <p class="text-sm md:text-lg">Сравнить товар</p>
                </div>
              </div>
              <p v-if="!prod_var" class="text-lg md:text-2xl mb-8">
                {{ detailProduct.price_range[0] }}₸ -
                {{ detailProduct.price_range[1] }}₸
              </p>
              <div v-else class="text-lg md:text-2xl mb-8">
                <div>
                  <div class="flex items-center gap-3">
                    ...
                    <span v-if="prod_var.discount_price"
                      >{{ prod_var.discount_price }}₸</span
                    >
                  </div>
                </div>
              </div>
              ...
              >
                <div class="rounded-full flex items-center justify-center">
                  <PlusIcon
                    class="w-7 h-7 bg-[#F0DFDF] text-mainColor rounded-full p-1"
                  />
                </div>
                <p class="text-sm md:text-xl text-mainColor">
              ...
              >
                ...
                <div class="">
                  ...
                  >
                    ...
                    >
                      <svg
                        v-if="favouriteColorIds?.includes(colorCookie.id)"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        class="size-6 text-mainColor"
                  ...
                  <div>
                    ...
                    <p class="text-sm text-mainColor">
              ...
              <div class="mb-8">
                <p class="mb-4 text-xs md:text-base">
                ...
                <div class="flex flex-wrap gap-3">
                  <button
                    ...
                    :key="index"
                    :class="[
                      { 'bg-mainColor text-white': form.variant === item.id },
                      {
                        '!border !border-red-500': v$.product_variant_id.$error,
                      },
                    ]"
                    class="transition-all px-4 md:px-6 w-max text-center text-base md:text-xl border border-[#7B7B7B] border-opacity-25 py-1 md:py-2 text-[#7B7B7B] rounded-lg"
                    @click="setData(item)"
                  >
              ...
              <div class="flex flex-row gap-6 mb-8">
                <div class="w-full md:w-1/3">
                  <p class="text-xs md:text-base mb-4">
                ...
                <div class="w-full md:w-2/3">
                  ...
                  <div
                    class="transition-all border-[#F0DFDF] text-base md:text-xl border px-5 py-3 rounded-lg flex items-center justify-center gap-3 cursor-pointer hover:bg-mainColor hover:text-white"
              ...
              <div class="mb-8">
                <p class="text-xs md:text-base">
                  {{ $t("products.details.norm") }}:
                  <span class="text-mainColor"
                    >{{ detailProduct.expense }} г/м2</span
                  >
                </p>
              </div>
              <div v-if="calculatorActive" class="mb-8">
                <div class="flex gap-4 items-end">
                  <div>
                    ...
                    <div class="relative">
                      ...
                      />
                      <p
                        class="absolute top-1/2 -translate-y-1/2 right-4 text-2xl"
                      >
                        м
                      </p>
                    </div>
                ...
                <div class="flex justify-between text-md md:text-xl mt-8">
                  <p class="text-xs md:text-base">
                    {{ $t("products.details.area") }}:
                    <span class="text-mainColor">{{ totalArea }} м²</span>
                  </p>
                  <p class="text-xs md:text-base">
                    {{ $t("products.details.on") }} {{ form.layers }}
                    {{ $t("products.details.layer") }}:
                    <span class="text-mainColor"
                      >{{ paintNeeded.toFixed(2) }}
                      {{ $t("products.details.liters") }}</span
                    >
                  </p>
                </div>
              </div>
              <button
                class="bg-white border border-mainColor text-sm md:text-xl font-montserrat text-mainColor rounded md:rounded-xl w-full py-2 md:py-4 transition-all hover:bg-mainColor hover:text-white"
    ...
    <div>
      <div class="container mx-auto px-4 md:px-0">
        <div
          class="flex flex-col md:flex-row justify-between items-start gap-10 md:gap-32"
        >
          <div class="w-full md:w-1/4">
            ...
            >
              <div
                class="p-6 rounded-t-xl bg-mainColor text-white font-semibold font-montserrat"
              >
              ...
              <div
                ...
                :key="index"
                :class="{
                  'border-b border-[#F0DFDF]':
                    index !== detailProduct.filter_data.length - 1,
                }"
                class="py-4 px-6 flex items-center gap-4"
              >
                <EyeDropperIcon
                  v-if="item.id === 10"
                  class="w-6 h-6 min-w-6 min-h-6 text-mainColor"
                />
                <Square3Stack3DIcon
                  v-if="item.id === 16"
                  class="w-6 h-6 min-w-6 min-h-6 text-mainColor"
                ...
                <div>
                  ...
                  <p class="text-sm text-[#7B7B7B] font-montserrat">
                    {{ item.value[cur_lang] }}
                  </p>
                </div>
              </div>
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <p class="text-sm">
                    {{ item.product.price_range[0] }}₸ -
                    {{ item.product.price_range[1] }}₸
                  </p>
                </div>
              </router-link>
            </div>
          ...
          >
            <div
              ...
              v-html="detailProduct.description[cur_lang]"
            ></div>
            <div>
              <p
                class="text-mainColor text-lg md:text-2xl font-medium font-montserrat mb-4"
              >
                {{ $t("products.details.serts") }}
              </p>
              <div>
                <a
                  :href="sert"
                ...
                >
                  <div class="flex gap-6">
                    ...
                    <p class="text-sm md:text-base">
                      {{ $t("products.details.sertificate") }}
                    </p>
                  </div>
    ...
    >
      <div class="flex flex-col md:flex-row items-center gap-10 md:gap-32 mb-6">
        <div class="w-full md:w-1/3 md:min-w-1/3">
          ...
          <div class="mt-4">
            <div
              v-for="([rating, ratingData], index) in Object.entries(
                productReviewsRating.rating.by_rating
              ).reverse()"
              :key="index"
              class="flex items-center gap-4 mb-2"
            >
              <div class="flex items-center gap-1">
                <span class="text-sm md:text-base font-bold">{{ rating }}</span>
                <svg
                  class="size-5 md:size-6 text-[#FFE814]"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                >
              ...
              <div class="w-full bg-[#EEEEEE] h-3 rounded">
                <div
                  ...
                  class="bg-[#FFE814] h-3 rounded"
                ></div>
              </div>
              <span class="text-sm md:text-base">{{ ratingData.value }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="w-full md:w-max">
          ...
          <div v-if="detailProduct.rating" class="flex gap-3 mb-4">
            <p class="text-6xl" v-if="detailProduct.rating">
              {{ detailProduct.rating.rating }}
            </p>
            <p class="text-6xl" v-else>0</p>
            <div class="flex flex-col justify-between">
              <div v-if="detailProduct.rating" class="flex gap-1">
                <svg
                  v-for="(item, index) of Math.round(
                    detailProduct.rating.rating
                  )"
                  :key="index"
                  class="size-6 text-[#FFE814]"
          ...
          <p class="text-sm md:text-base">
            {{ products.productReviewsRating.recommendation.recommended }}
            {{ $t("products.details.out") }}
            {{ products.productReviewsRating.recommendation.total }}
            ({{ products.productReviewsRating.recommendation.percentage }}%)
            {{ $t("products.details.outOfTotal") }}
          </p>
        </div>
      </div>
      <div class="flex flex-col gap-3 w-full md:w-1/3">
        <h3 class="text-lg md:text-xl font-bold">
          {{ $t("products.details.share.title") }}
        </h3>
        <p class="text-sm md:text-base">
          {{ $t("products.details.share.subtitle") }}
        </p>
        <p
          @click="modals.showModal('addReview', detailProduct.id)"
          class="w-full flex text-sm md:text-base items-center justify-center border border-mainColor text-mainColor py-3 rounded-lg hover:text-white hover:bg-mainColor transition-all cursor-pointer"
        >
    ...
    <div v-if="products.productReviews" class="container mx-auto px-4 md:px-0">
      <div v-if="reviews.data.length > 0" class="mt-10">
        <div class="flex items-center gap-2 mt-4 border-b pb-5">
          <p class="text-sm md:text-base">
            {{ reviews.data.length }} {{ $t("products.details.reviewsTotal") }}
          </p>
        </div>
        ...
        <div
          ...
          :key="review.id"
          class="py-6 border-b border-[#F0DFDF] flex flex-col md:flex-row items-start"
        >
          <div class="block md:flex items-center gap-4 w-full md:w-1/5">
            <img class="w-9 h-9" src="@/assets/img/avatar.png" alt="" />
            <div class="flex flex-row md:flex-col gap-4 mb-5 md:mb-0">
              <div class="flex flex-col">
                <p class="text-sm md:text-xl font-bold mb-2">
                  {{ review.user.name }}
                </p>
                <div class="flex gap-1">
                  <svg
                    ...
                    :key="index"
                    class="size-4 md:size-6 text-[#FFE814]"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                  ...
                  <svg
                    ...
                    :key="index"
                    class="size-4 md:size-6 text-[#D6D6D6]"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                  >
              ...
              <div class="hidden md:flex gap-4">
                ...
                <div class="flex gap-3 items-center">
                  ...
                  />
                  <p class="font-bold">
                    ({{ review.helpful_data.not_helpful }})
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="w-full md:w-4/5 flex flex-col gap-4">
            <p class="text-sm md:text-xl pb-3 border-b border-[#F0DFDF]">
              {{ $t("products.details.review") }}:
            </p>
            <p class="italic text-sm md:text-base">"{{ review.comment }}"</p>
            <div>
