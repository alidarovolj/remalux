...
<template>
  <div class="pb-16">
    ...
    <Banner
      ...
      text_color="main"
    />
    <div v-if="newsList" class="container mx-auto px-4 lg:px-0">
      <div class="flex flex-col gap-10">
        <div
          v-for="(news, index) in newsList.data"
          :key="news.id"
          :class="{
            'flex-col md:!flex-row-reverse !bg-[#F5F5F5] text-check':
              index % 2 === 0,
          }"
        ...
        >
          <img
            :src="news.image_url"
            alt=""
            class="w-full md:w-1/2 min-w-[50%] h-[200px] md:h-[414px] object-cover rounded-t-lg"
          />
          <div class="p-5 md:p-14">
            <h2 class="text-base md:text-xl font-semibold mb-4 font-montserrat">
              {{ news.title[cur_lang] }}
            </h2>
            <p
              class="text-xs md:text-sm text-gray-500 mb-8"
              v-html="news.description[cur_lang]"
            ></p>
            <nuxt-link
              :to="localePath(`/news/${news.id}`)"
              class="text-sm md:text-base text-mainColor border-2 border-mainColor py-2 px-7 rounded"
            >
              {{ $t("news.read_more") }}
            </nuxt-link>
          </div>
