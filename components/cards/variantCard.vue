...
import {ChevronRightIcon, HeartIcon} from "@heroicons/vue/24/outline";
...
<template>
  <div
      ...
      class="mt-32 w-full relative">
    <NuxtLink
        :to="localePath('/store/' + productData.product.id)"
        class="w-full hover:shadow-hovShadow h-full product-hover bg-white block rounded-md relative pb-3 text-left transition-all relative pt-24 hover:-translate-y-3"
    ...
    >
      ...
      <div class="absolute right-3 top-3 p-2 w-8 h-8 rounded-full bg-[#F0DFDF] flex items-center justify-center">
        <svg
            v-if="productData.product.is_colorable" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 text-mainColor">
      ...
      <div class="px-4 flex flex-col gap-2 relative">
        <div class="flex justify-between items-center">
          ...
          <p class="py-1 px-3 w-max border border-mainColor rounded-xl">
            Вес: {{ productData.value }}{{ $t('products.details.kg') }}.
          </p>
        </div>
